...
const COLORS = [
  ...
  '#6366F1', '#14B8A6', '#F97316', '#8B5CF6', '#64748B'
];
...
const PortfolioDetails: React.FC = () => {
  ...
  const fetchPortfolioData = async () => {
    ...
    try {
      const [holdingsResponse, analyticsResponse] = await Promise.all([
        fetch(`/api/holding/portfolios/${id}/holdings`, {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('investmind_token')}`
          }
        }),
        fetch(`/api/portfolios/${id}/analytics`, {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('investmind_token')}`
          }
        })
  ...
  const handleAddHolding = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/holding/portfolios/${id}/holdings`, {
        method: 'POST',
        headers: {
  ...
  const handleDeleteHolding = async () => {
    ...
    setIsSubmitting(true);
    try {
      const response = await fetch(`/api/holdings/${holdingToDelete}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('investmind_token')}`
        }
  ...
  const handleExport = async () => {
    ...
    try {
      const response = await fetch(`/api/exports/${id}/export/excel`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('investmind_token')}`
        }
      });
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 space-y-4 sm:space-y-0">
            <div className="flex items-center space-x-4">
              <button
                ...
                className="flex items-center space-x-2 bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-lg transition-all duration-300"
              >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                          ...
                          <tr key={holding.id} className="hover:bg-gray-750">
                            ...
                            <td className="px-6 py-4">
                              <span className={`font-mono ${
                                holding.gainLossPercentage >= 0 ? 'text-green-500' : 'text-red-500'
                              }`}>
            ...
            <div className="bg-gray-800/50 backdrop-blur-sm rounded-xl p-6 border border-gray-700">
              ...
              ) : !analytics?.sectorBreakdown?.length ? (
                ...
                <div className="h-64">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      ...
                      >
                        {analytics.sectorBreakdown.map((_, index) => (
                            ...
                            return (
                              <div className="bg-gray-800 p-3 rounded-lg border border-gray-700">
                                ...
                                <p className="text-sm font-semibold text-white">
                                  {formatPercentage(payload[0].value * 100)}
                                </p>
                              </div>
                            );
                          }
